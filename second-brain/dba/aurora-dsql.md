...
topics: [dba]
summary: >-
  Aurora DSQL 핵심 특성과 공식 한도 — OCC(SQLSTATE 40001) 앱 리트라이 필수, FK/Trigger/PL-pgSQL/
  TRUNCATE 미지원, 클러스터당 DB 1개·스키마 10개, 트랜잭션 3000행/10MiB/5분, DPU 과금.
...
- **DB가 1개뿐**이므로 논리 분리는 스키마(최대 10개) 또는 **클러스터를 나누는 것**으로 해결. 스키마 단위 분리 시 `search_path` 운영은 [[dba/postgresql-operations]]의 PG 규칙과 동일하게 적용된다.^[inferred]
