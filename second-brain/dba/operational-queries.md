...
sources:
  - "표준 시스템 뷰·카탈로그 기반 작성 (second-brain session 2026-08-04)"
category: dba
lifecycle: draft
tier: core
created: 2026-08-04
...
- **섹션 1~11과 12는 읽기 전용(SELECT/SHOW), 섹션 13~15는 변경 명령(GRANT/DDL/DML)이다.** 이 경계를 페이지 구조로 분리한 것은 의도적이다 — [[dba/notion-remediation-backlog]]에 "통계 리셋 명령이 조회 쿼리 사이에 섞여 있음", "조각화율만으로 REBUILD 스크립트 생성"이 교정 대상으로 잡혀 있다. **조회와 변경을 같은 블록에 섞지 않는다.**
...
- 임계값은 **일반론이며 반드시 기준선 보정**한다 — [[dba/monitoring-incident-runbook]]의 원칙과 동일.
- 조회 순서는 [[dba/monitoring-incident-runbook]]의 감시 흐름을 따른다: CloudWatch → PI/Database Insights → **엔진 내부 뷰(이 페이지)** → 슬로우 쿼리·실행 계획 → 조치.
...
`idle in transaction` 단독 확인 — bloat 장애 1순위 원인이므로 별도로 본다([[dba/postgresql-operations]]):
...
장기 트랜잭션 (Undo 폭증·HLL 증가 원인 추적 — [[dba/mysql-operations]]):
...
> `sys.sysprocesses`는 하위호환용 구식 뷰다. `blocked` 컬럼은 **차단자의 SPID이며 임계치가 아니다** — [[dba/notion-remediation-backlog]]에 오독 사례로 등록돼 있다.
...
> `blocked process report`를 쓰려면 `sp_configure 'blocked process threshold'`가 1초로 설정돼 있어야 하고, 이벤트는 Extended Events로 수집한다 — [[dba/sqlserver-operations]].
...
> `rows_examined / rows_sent` 비율이 크면 인덱스 미사용 신호 — 실행 계획 확인으로 넘어간다([[dba/mysql-operations]]).
...
> `dm_exec_query_stats`는 **플랜 캐시가 축출되면 사라진다.** 시계열 회귀 분석은 Query Store(2016+, 2022부터 기본 활성)가 정확하다 — [[dba/sqlserver-operations]]의 3차 대응 단계에서 쓰는 도구.
...
> `Threads_running`이 실제 부하 지표다(`Threads_connected`는 유휴 포함) — [[dba/mysql-operations]].
...
**PostgreSQL** — dead tuple 비율. [[dba/postgresql-operations]]의 P2 알람 기준(dead 20~30% + `n_live_tup>100k`)과 같은 조건:
...
> **이 결과로 REBUILD 스크립트를 자동 생성하지 않는다.** 조각화율 단독 판단은 [[dba/notion-remediation-backlog]]에 교정 대상으로 등록된 안티패턴이다. `DETAILED` 모드는 전체 스캔이므로 운영 시간대 금지.
...
**PostgreSQL** — XID wraparound. 알람 기준은 10억(경보)/15억/18억, 20억에서 강제 셧다운([[dba/postgresql-operations]]):
...
> `SHOW ENGINE INNODB STATUS`의 TRANSACTIONS 섹션에서도 같은 값을 볼 수 있다. 수십만 이상이면 장기 트랜잭션이 Purge를 막고 있는 상태다. `innodb_purge_threads`·`batch_size`는 **원인 확인 전 조정 금지** — [[dba/mysql-operations]].
...
> [[dba/postgresql-operations]]에 기록된 교정 사항 — 원본 문서의 MySQL 히트율 쿼리에 PG 전용 `FILTER (WHERE ...)` 문법이 섞여 있어 MySQL에서 실행 불가였다. 위 쿼리는 그 오류를 피한 형태다.

**SQL Server** (PLE 현대 기준 1,000~3,000초+, Memory Grants Pending = 0 — [[dba/sqlserver-operations]])
...
> `CXPACKET` 상위면 MAXDOP·Cost Threshold 검토, 컴파일 폭주면 `OPTIMIZE FOR AD HOC` + 파라미터화 — [[dba/sqlserver-operations]]의 CPU 100% 대응 흐름.

## 12. 권한 감사 (읽기 전용)

**공통 원칙** — [[dba/db-access-control]]의 표준과 짝을 이룬다. 권한 변경 후에는 **반드시 실측 조회로 검증**한다(부여 성공 ≠ 의도한 권한).
...
**DEFAULT PRIVILEGES 확인** — `GRANT ON ALL TABLES`는 현재 객체만 적용되므로([[dba/postgresql-operations]]) 이 조회가 실제 운영 여부를 가른다:
...
**설계 원칙** ([[dba/db-access-control]], [[dba/postgresql-operations]])
...
모니터링 계정 (조회 전용 — [[dba/db-access-control]]):
...
sa 비활성화 + 명명 관리자 계정 ([[dba/sqlserver-operations]]):
...
파티션 인덱스 — 부모는 `CONCURRENTLY`를 지원하지 않으므로 자식별로 만들어 붙인다 ([[dba/postgresql-operations]]):
...
> 임의 위치 INSTANT ADD COLUMN은 8.0.29부터지만, **8.0.29는 회수 릴리스로 사용 금지**다([[dba/mysql-operations]]). 8.0.30+ 또는 8.4 LTS를 쓴다.
...
> **조각화율만 보고 REBUILD 대상을 자동 생성하지 않는다.** 7번 섹션 조회로 대상을 확인하고, `page_count > 1000`·조각화 30%+에 한해 REBUILD, 5~30%는 `REORGANIZE`를 검토한다. `MAXDOP = 0`(전체 코어)과 `ONLINE` 누락 조합이 [[dba/notion-remediation-backlog]]의 교정 대상이다.
...
청크 DELETE — **청크마다 커밋한다.** 하나의 긴 트랜잭션은 VACUUM cutoff를 붙잡아 전체 DB의 데드 튜플 정리를 막는다([[dba/postgresql-operations]]):
...
청크 DELETE 루프 ([[dba/sqlserver-operations]]):
...
UPSERT는 `MERGE` 대신 `UPDLOCK` + `HOLDLOCK` — `MERGE`는 동시성 버그로 운영 비권장([[dba/sqlserver-operations]]):
...
> `TRUNCATE`는 이 페이지에 넣지 않았다. 최소 로깅이라 빠르지만 DDL에 가깝고 롤백 조건·FK 제약이 다르며, "TRUNCATE 생성기"가 [[dba/notion-remediation-backlog]] P1 교정 대상이다. 필요하면 대상·복구 계획을 명시한 별도 절차서로 만든다.
...
- 미수록: 파티션 현황, 통계 최신성(마지막 ANALYZE/UPDATE STATISTICS), 백업 이력 조회, 권한 감사 쿼리, Aurora 전용 지표 뷰.
- [[dba/aurora-dsql]]은 이 페이지 대상이 아니다 — 시스템 카탈로그가 제한적이고 VACUUM·bloat·복제 지연 개념 자체가 없다.
