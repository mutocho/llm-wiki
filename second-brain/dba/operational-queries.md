...
- **전부 읽기 전용(SELECT/SHOW)이다.** DDL·DML·리셋 명령은 의도적으로 넣지 않았다 — [[dba/notion-remediation-backlog]]에 "통계 리셋 명령이 조회 쿼리 사이에 섞여 있음", "조각화율만으로 REBUILD 스크립트 생성"이 교정 대상으로 잡혀 있고, 같은 실수를 반복하지 않기 위한 원칙이다.
